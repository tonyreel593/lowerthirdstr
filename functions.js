...
<body>
    ...
    <script>
        ...
        function showLowerThird(index) {
            ...
            // Si la animación se repite indefinidamente en el MISMO lower third (sin rotar a otros)
            if (lt.loopAnimation && lt.loopInterval && (!data.rotation.enabled || data.lowerThirds.length === 1)) {
                // Loop infinito en el mismo lower third
            ...
            // Si la rotación automática está activada Y hay múltiples lower thirds
                ...
                // Determinar el tiempo de visualización
                let duration;
                if (lt.displayDuration) {
                    // Si tiene duración de visualización individual, úsala
                    duration = parseInt(lt.displayDuration) * 1000;
                } else {
                    // Usa el intervalo global de rotación
                    duration = data.rotation.interval * 1000;
                }

                // Programar el siguiente lower third
                rotationTimer = setTimeout(() => {
                    currentIndex = (currentIndex + 1) % data.lowerThirds.length;
                    showLowerThird(currentIndex);
                }, duration);
            }
            // Si NO hay rotación pero SÍ hay loop individual, quedarse mostrando este lower third
            else if (lt.loopAnimation && lt.loopInterval) {
                const loopDuration = parseInt(lt.loopInterval) * 1000;
                rotationTimer = setTimeout(() => {
                    const element = container.querySelector('.lower-third');
                    if (element) {
            ...
            // Si no hay rotación ni loop, el lower third se queda estático
        }
